...
| <a name="input_ecr_repos"></a> [ecr\_repos](#input\_ecr\_repos) | The set of ecr repos (i.e. service types) and namespaces (i.e. release/dev) to create. | <pre>list(object({<br/>    name                    = string<br/>    namespace               = optional(string, "")<br/>    immage_tag_immutability = optional(string, "IMMUTABLE")<br/>    image_tag_mutability_exclusion_filter = object({<br/>      filter      = string<br/>      filter_type = optional(string, "WILDCARD")<br/>    })<br/>  }))</pre> | `[]` | no |
